...
public class CoreClient {
  ...
  protected static void setLoggingContext(String suppliedUName, String suppliedServerToConnect,
      boolean _isAdmin) {
    isAdmin = _isAdmin;
    uName = suppliedUName;
  ...
  protected static String getIpAddressInput() {
    ...
    String regionServer;
    while ((regionServer = CoreClient.getRegionServer(ipAddress)) == null
        || !regionServer.startsWith("GameServer")) {
      if (regionServer != null && regionServer.equals("Unknown Server")) {
        String err = "ERROR: Server for this region does not exist!";
        System.out.println(err);
        if (isAdmin)
          adminLog(err, uName, serverToConnect);
        else
  ...
  protected static synchronized void playerLog(String logStatement, String uName,
      String ipAddress) {
    DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss");
    LocalDateTime tStamp = LocalDateTime.now();
    String writeString =
        String.format("[%s] %s @ (%s) -- %s", dtf.format(tStamp), uName, ipAddress, logStatement);
    String serverRegion = getRegionServer(ipAddress);
    String fName =
        serverRegion == null || serverRegion.equals("Unknown Server") ? "UNRESOLVED-Players"
            : serverRegion.substring(10);
    try {
  ...
  protected static synchronized void adminLog(String logStatement, String uName,
      String serverToConnect) {
    ArrayList<String> REGION_LIST =
        new ArrayList<String>(Arrays.asList("GameServerNA", "GameServerEU", "GameServerAS"));
    if (!REGION_LIST.contains(serverToConnect))
      serverToConnect = "GameServerUNRESOLVED";
    DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss");
    LocalDateTime tStamp = LocalDateTime.now();
    String writeString = String.format("[%s] %s @ (Admin-%s) -- %s", dtf.format(tStamp), uName,
        serverToConnect.substring(10), logStatement);
    try {

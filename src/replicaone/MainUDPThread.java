...
class MainUDPThread extends Thread {
  ...
  public static void main(String[] args) throws InterruptedException {
    String[] defaultArgs = {};
    replicaOne = new MainUDPThread(REPLICA_ONE_PORT, defaultArgs);
    while (true) {
      ...
      if (replicaOne.replicaManagerListener.hasCrashed()) {
        ...
        try {
          replicaOne.replicaManagerListener = new ReplicaManagerListenUDPThread(REPLICA_ONE_PORT);
        } catch (SocketException e) {
  ...
  private MainUDPThread(int port, String[] args) {
    ...
    try {
      ...
      replicaManagerListener = new ReplicaManagerListenUDPThread(REPLICA_ONE_PORT);
  ...
  private void createRegionServerInstances() throws UnknownServerRegionException {
    NAGameServer =
        new GameServerServant(R1_NA_NAME, new ArrayList<>(Arrays.asList(7990, 7991)), 7989);
    EUGameServer =
        new GameServerServant(R1_EU_NAME, new ArrayList<>(Arrays.asList(7989, 7991)), 7990);
    ASGameServer =
        new GameServerServant(R1_AS_NAME, new ArrayList<>(Arrays.asList(7989, 7990)), 7991);
  }
